...
class MainApp:
    ...
    def add_to_frame_queue(self,frame):
        ...
        if self.frame_queue.qsize() > 100:  # Adjust this number based on your needs
            try:
                # Remove old frames
    ...
    {"<br>" + cpu if cpu else ""} {" " + gpu if gpu else ""} {" " + ram if ram else ""} {" " + ssd if ssd else ""}
        ...
        runcommand(
            f"'{CHROME}' --headless --disable-gpu --no-pdf-header-footer --print-to-pdf='{pdf_path}' '{html_path}'")
    ...
    def main_check(serial_number, bypass):
        ...
        if not main_check_lock.acquire(blocking=False):
            print(
                f"\033[93mAnother main_check is already running for serial {serial}, skipping {serial_number}...\033[0m")
            return
        ...
        try:
            ...
            if check_type:
                ...
                if icloudInfo:
                    ...
                    log_event(
                        f"iCloud MDM Check: {serial_number} | CPU: {cpu} | GPU: {gpu} | RAM: {ram} | SSD: {ssd} | iCloud: {icloud} | MDM: {mdm} | Config: {config} | Model: {model_name_sickw} ")
                    print(
                        f"\033[91miCloud: {icloud} | MDM: {mdm} | Config: {config} | Model: {model_name_sickw}\033[0m")
